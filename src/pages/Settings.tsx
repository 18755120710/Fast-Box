    ...
    <div className="space-y-8 animate-fade-in font-mono text-xs">
      <div>
        <h2 className="text-xl font-bold text-slate-100 flex items-center gap-2">
          Settings <SettingsIcon className="h-5 w-5 text-accent" />
        </h2>
        <p className="text-[10px] text-slate-500 mt-0.5">Configure target compilation variables and environment path links.</p>
      </div>
      ...
      <div className="border border-slate-800 bg-slate-950 p-5 rounded space-y-4">
        <h3 className="font-bold text-slate-200 uppercase tracking-wider text-[10px] border-b border-slate-900 pb-2">
          FAST BOX ROOT DIRECTORY
        </h3>
        <div className="space-y-2">
          <label className="text-slate-400 block">Workspace Path</label>
          ...
          <p className="text-[10px] text-slate-500">
            Isolated node binaries, versions and symlink configurations will be managed inside this location.
          </p>
        </div>
      </div>
      ...
      <div className="border border-slate-800 bg-slate-950 p-5 rounded space-y-4">
        <h3 className="font-bold text-slate-200 uppercase tracking-wider text-[10px] border-b border-slate-900 pb-2">
          REGISTRY PREFERENCES (CHINA ACCELERATION)
        </h3>
        <div className="space-y-3">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <label className="flex-1 border border-slate-800 bg-slate-900/40 hover:bg-slate-900 hover:border-accent/40 rounded p-3 flex items-center justify-between cursor-pointer transition-colors duration-150">
              <div className="space-y-1">
                <span className="font-bold text-slate-200">Huawei Cloud Mirror</span>
                <p className="text-[9px] text-slate-500">Recommended speed inside mainland China.</p>
              </div>
              <input
                type="radio"
                name="registry"
            ...
            <label className="flex-1 border border-slate-800 bg-slate-900/40 hover:bg-slate-900 hover:border-accent/40 rounded p-3 flex items-center justify-between cursor-pointer transition-colors duration-150">
              <div className="space-y-1">
                <span className="font-bold text-slate-200">Nodejs Official</span>
                <p className="text-[9px] text-slate-500">Original download path nodes.org.</p>
              </div>
              <input
                type="radio"
                name="registry"
          ...
          <div className="space-y-2 pt-2">
            <label className="text-slate-400 block">Active NPM Registry Preference</label>
            <input
              type="text"
            ...
            />
            <p className="text-[10px] text-slate-500">
              This registry address will be automatically populated in the post-install npm configuration.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="border border-yellow-500/20 bg-yellow-500/5 p-5 rounded space-y-4">
        <h3 className="font-bold text-yellow-500 uppercase tracking-wider text-[10px] flex items-center gap-2">
          ...
          SHELL PATH CONFIGURATION (REQUIRED)
        </h3>
        <p className="text-[10px] text-slate-300 leading-relaxed">
          For your active Node.js versions to execute correctly in your terminal, the Fast Box shims path must be loaded in your system environment profile.
        </p>
        ...
        <div className="space-y-2 text-[10px] text-slate-400 pt-1">
          <div className="font-semibold text-slate-300">How to apply:</div>
          <ol className="list-decimal list-inside space-y-1">
            <li>Copy the export command box above.</li>
            <li>Open your shell config file (e.g. <code className="text-slate-300 bg-slate-900 px-1 py-0.5 rounded">~/.zshrc</code> or <code className="text-slate-300 bg-slate-900 px-1 py-0.5 rounded">~/.bash_profile</code>).</li>
            <li>Paste the line at the very bottom and save.</li>
            <li>Restart your terminal or run <code className="text-slate-300 bg-slate-900 px-1.5 py-0.5 rounded">source ~/.zshrc</code>.</li>
          </ol>
        </div>
      </div>
    </div>
