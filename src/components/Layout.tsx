...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  const { currentTab, setCurrentTab, systemInfo, activeTask } = useApp();

  const menuItems = [
    { id: 'home', label: 'Dashboard', icon: Terminal },
    { id: 'env', label: 'Environments', icon: Layers },
    { id: 'progress', label: 'Task Progress', icon: Play, badge: activeTask.status === 'running' },
        ...
        <div>
          ...
          <div className="h-16 flex items-center px-6 border-b border-slate-900 gap-3">
            ...
            <div>
              ...
              <span className="text-[10px] text-accent font-semibold">DEV TOOLCHAIN</span>
            </div>
          </div>
        ...
        <div className="p-4 border-t border-slate-900 bg-slate-950/80 text-[10px] space-y-3">
          <div className="space-y-1.5 text-slate-400">
            <div className="flex items-center gap-2">
              ...
              <span className="truncate">Arch: <span className="text-slate-200">{systemInfo?.arch || 'detecting...'}</span></span>
            </div>
            <div className="flex items-center gap-2">
              <HardDrive className="h-3.5 w-3.5 text-slate-500" />
              <span className="truncate" title={systemInfo?.fastboxHome}>
                Home: <span className="text-slate-200">{systemInfo?.fastboxHome ? '~/.fastbox' : 'detecting...'}</span>
              </span>
            </div>
          </div>
          <div className="flex items-center justify-between pt-2 border-t border-slate-900">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] text-slate-500">CORE CONNECTED</span>
            </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-slate-900/40">
        ...
        <header className="h-16 border-b border-slate-800 bg-slate-950/30 flex items-center justify-between px-8 select-none">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-400">
            <span>{currentTab.startsWith('detail-') ? 'Environment Detail' : currentTab}</span>
            {activeTask.status === 'running' && (
              <span className="text-[10px] lowercase text-accent font-normal bg-accent/10 px-2 py-0.5 rounded border border-accent/20 animate-pulse">
                installing: {activeTask.packageName}@{activeTask.version}
              </span>
            )}
          </div>
          <div className="text-xs text-slate-500">
            Press <kbd className="bg-slate-800 text-slate-300 px-1 py-0.5 rounded text-[10px] border border-slate-700">⌘,</kbd> for Settings
          </div>
        </header>
